...
namespace DTT.MinigameMemory
{
    ...
    public class Board : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField] [Tooltip("GridLayoutGroup used to position the cards.")]
        private GridLayoutGroup _grid;

        /// <summary>
        /// The space between the cards in percentage of card size.
        /// </summary>
        [SerializeField] [Tooltip("Percentage of card size used for space between te cards.")]
        ...
        /// </summary>
        [SerializeField] private float _maximumCardSize = 0f;

        /// <summary>
        /// Prefab to make cards from.
        /// </summary>
        [SerializeField] [Tooltip("Prefab used to create the cards shown on the board.")]
        ...
        private void SetupGrid(int numberOfCards)
        {
            ...
            while (
                ...
                (isLandscapeOrientation
                    ? ((occupiedColumns % 2 == 0 && occupiedColumns / 2 != 1) ||
                       (occupiedColumns % 3 == 0 && occupiedColumns / 3 != 1))
                    : ((occupiedRows % 2 == 0 && occupiedRows / 2 != 1) ||
                       (occupiedRows % 3 == 0 && occupiedRows / 3 != 1))
                )
            )
            {
        ...
        private void RemoveMatchedCards(Card card1, Card card2)
        {
            ...
            if (_inactiveCards.Count >=
                Mathf.FloorToInt(_cardsOnBoard.Count * (_activateAtMatchesFoundPecentage / 100f)))
                ActivateCards();
